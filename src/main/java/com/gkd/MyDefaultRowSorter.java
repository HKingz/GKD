...
public abstract class MyDefaultRowSorter<M, I> extends RowSorter<M> {
	public int showAfterwardCount;
	...
	private void initializeFilteredMapping() {
		...
		for (i = 0; i < rowCount; i++) {
			if (include(i)) {
				...
				for (int x = 0; x < showAfterwardCount; x++) {
					if (i < rowCount - 1) {
						i++;
						modelToView[i] = i - excludedCount;
					}
	...
	private static class Row implements Comparable<Row> {
		private MyDefaultRowSorter sorter;
		int modelIndex;

		public Row(MyDefaultRowSorter sorter, int index) {
			this.sorter = sorter;
			modelIndex = index;
		}
